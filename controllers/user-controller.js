...
const userController = {
  ...
  putUser: (req, res, next) => {
    ...
    const { file } = req
    if (!name) throw new Error("User name is didn't exist")
    return Promise.all([
      User.findByPk(id),
  ...
  addFollowing: (req, res, next) => {
    ...
    Promise.all([
      ...
      .then(([user, followship]) => {
        if (!user) throw new Error('User is did not exist')
  ...
  removeFollower: (req, res, next) => {
    Followship.findOne({
      ...
      .then(followship => {
        if (!followship) throw new Error('You did not followed this user!')
